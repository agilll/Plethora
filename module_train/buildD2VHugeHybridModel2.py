...
# read the first x% from 1926.ph5-3.simsBest.csv: the list of ad hoc candidates
...
sizeAHCorpus = int(len(listAP) / 100) *  ipctg   # x% of the total candidates
...
class MyCorpusBoth():
	...
	def __iter__(self):
		...
		print("\nAd hoc files")
		for i,file in enumerate(listCorpusAH):
			if (i % 100) == 0:
				print(i, end=' ', flush=True)
...
model.train(corpus_data_both, total_examples=model.corpus_count, start_alpha=alpha, end_alpha=min_alpha, epochs=epochs1)  # hay 408 docs txt en el corpus gen
model.save(MODEL_NAME)
print("Terminado", MODEL_NAME, epochs1, "epochs, vector size=", vector_size)
