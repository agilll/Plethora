...
if os.path.isfile(origin):
	if not origin.endswith(".s"):
		print("The file "+origin+" has not '.s' extension")
		exit(-1)
	print("Processing file "+origin+"...\n")
	result = getContentAfterChanges(origin, origin+".p")
	# save result in files with the same name a new extensions  
	_saveFile(origin+".w", result[0])   # the new text with extensióo '.w'
	_saveFile(origin+".w.html", result[1])   # the report with the changes with extension '.w.html'
	
	highlightedContent = _getContentMarked(origin+".w", "w")
	_saveFile(origin+".w.p.html", highlightedContent)
